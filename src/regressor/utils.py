...
def get_GV(directory="../../data/GAIED/validator_data"): # This gives the GV array
...
def get_precision(directory="../../data/GAIED/generator_data"):
        ...
        for q in data:
            try:
                for line in q['feedback']:
                    if line['category'] in ['TP', 'FP-R', 'FP-E']:
                        v += 1
    ...
    for file in os.listdir(directory):
        if file.endswith('.json'):
            v, iv = get_true_precision(directory + '/' + file)

            try:
                gen = file.split('_feedback')[0].strip()
                pGa[gen] = v / (v + iv)
            except:
                pass
    return pGa